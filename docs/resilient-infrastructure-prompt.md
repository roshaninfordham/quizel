...
- Soft cap: 100 active racers.
- Hard cap: 100 active racers.
...
Capacity path:
- Current deployed implementation is measured at hard cap 100 active racers.
- The current active-user artifact passes 100 connected users with 100 admitted, 1000/1000 answers, 100 FinalResult rows, and 100 current ShareCard rows.
- The 250 connected-user artifact does not pass yet; it joined and waitlisted correctly but produced late/failed admitted answer commits under pressure.
- The next engineering target is 250 connected audience reliability after `LeaderboardTopN`, persisted bracket rows, and reduced `MatchEvent` fanout.
- Do not claim 250 active racers, 250 audience readiness, or 1000 active racers until load:prod passes and docs/capacity-results contains the artifact.
